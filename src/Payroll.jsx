...
export const SAMPLE_STAFF=[
  ...
  {id:'s5',name:'VOON SUK YIN',ic:'001028-13-1446',position:'INVOICING CLERK',salary:1950,method:'cash',status:'permanent',defIncentive:150,bankAcc:'CIMB 7630025322'},
  {id:'s6',name:'CHAI WAN NEE',ic:'011227-13-0648',position:'ASST. CASHIER SUPERVISOR',salary:1950,method:'cash',status:'permanent',defIncentive:150,bankAcc:'RHB 11110000208127'},
...
async function exportExcel(mo,yr,bR,cR,bT,cT,gT,ptR,ptT,notes,bL,sb=true){
  ...
  const mc=c=>sb?c:(c>6?c-1:c),L=mc(17);
  ...
  let row=5,sn=1;
  const _bankAmt=s=>Math.round((s.salary-s.epfP-s.socsoP-s.eisE-(s.advance||0))*100)/100;
  const _cashAmt=s=>Math.round(((s.incentive||0)+(s.bonus||0))*100)/100;
  const wR=(rows,isBank)=>{rows.forEach(s=>{if(s.addedMonth===viewMK)hlRows.add(row);sc(row,0,sn);sc(row,1,s.name);sc(row,2,s.ic);sc(row,3,s.position);sc(row,4,s.salary);sc(row,5,s.incentive||0);sc(row,6,s.bonus||0);sc(row,7,s.epfM);sc(row,8,s.epfP);sc(row,9,s.epfM+s.epfP,`${A1(row,7)}+${A1(row,8)}`);sc(row,10,s.socsoM);sc(row,11,s.socsoP);sc(row,12,s.socsoM+s.socsoP,`${A1(row,10)}+${A1(row,11)}`);sc(row,13,s.eisE);sc(row,14,s.eisE*2,`${A1(row,13)}*2`);sc(row,15,s.advance||0);if(isBank){sc(row,16,_bankAmt(s),`${A1(row,4)}-${A1(row,8)}-${A1(row,11)}-${A1(row,13)}-${A1(row,15)}`);sc(row,17,_cashAmt(s),`${A1(row,5)}${sb?'+'+A1(row,6):''}`)}else{sc(row,16,s.netPay,`${A1(row,4)}+${A1(row,5)}${sb?'+'+A1(row,6):''}-${A1(row,8)}-${A1(row,11)}-${A1(row,13)}-${A1(row,15)}`);mg.push({s:{r:row,c:mc(16)},e:{r:row,c:L}});}sn++;row++;});};
  ...
  mg.push({s:{r:cashSub,c:mc(16)},e:{r:cashSub,c:L}});
  sc(row,0,'GRAND TOTAL:');mg.push({s:{r:row,c:0},e:{r:row,c:3}});
  ...
  const cols=[5,35,18,32,14,12,14,10,10,12,10,10,14,10,12,10,12,10];if(!sb)cols.splice(6,1);
  ws['!cols']=cols.map(w=>({wch:w}));
  ws['!margins']={left:0,right:0,top:0.7874,bottom:0.7874,header:0,footer:0};
...
@media print{
  ...
  .t:not(.nb) col:nth-child(2){width:18%!important}    /* Name */
  ...
  .t.nb col:nth-child(2){width:18%!important}    /* Name */
  ...
  .t .tr td{background:#EFF6FF!important;font-size:7pt!important;font-weight:700;border-top:1px solid #000!important;border-bottom:1px solid #000!important;white-space:nowrap!important;overflow:visible!important;text-overflow:clip!important;-webkit-print-color-adjust:exact;print-color-adjust:exact}
  .t .gr td{background:#F0FDF4!important;font-size:7pt!important;font-weight:700;border-top:1px solid #000!important;border-bottom:1px solid #000!important;white-space:nowrap!important;overflow:visible!important;text-overflow:clip!important;-webkit-print-color-adjust:exact;print-color-adjust:exact}
  ...
  @page{size:A4 landscape;margin:10mm 0 0 0}
}
.po{display:none}
...
export default function Payroll({canUndo, onUndo, canRedo, onRedo}){
  ...
  const[remAll,setRemAll]=useState(()=>{const r=loadJ(LS_R,{});const jk='2026-07';if(!(r[jk]||[]).some(x=>/SWEE EN/i.test(x))){r[jk]=[...(r[jk]||[]),'INACTIVE STAFF: JEE SWEE EN'].filter(Boolean);}return r;});
  ...
  const notes=useMemo(()=>{
    ...
    const inactiveNames=hiddenStaff.filter(s=>{const fm=Object.entries(hidden).filter(([,ids])=>Array.isArray(ids)&&ids.includes(s.id)).map(([m])=>m).sort()[0];return fm===mk;}).map(s=>s.name);
    if(inactiveNames.length)n.push(`INACTIVE STAFF: ${inactiveNames.join(', ')}`);
    return n;
  ...
  const reorderStaff=(fromId,toId)=>{
    ...
    setStaff(p=>{
      ...
      let moved=picked;
      if(picked.status==='probationary'&&target.status!=='probationary')
        moved={...moved,status:'permanent'};
      if(picked.method!==target.method)
        moved={...moved,method:target.method};
  ...
  const COLNM={4:'Basic Salary',5:'Incentive',6:bl||'Bonus',7:'EPF (Majikan)',8:'EPF (Pekerja)',9:'Jumlah EPF',10:'SOCSO (Majikan)',11:'SOCSO (Pekerja)',12:'Jumlah SOCSO',13:'EIS',14:'Jumlah EIS',15:'Advance',16:'Net Pay',17:'Cash'};
  const selSum=(l,c,v)=>{
    const nm=COLNM[c]||'';
    const f = l==='GRAND TOTAL:' ? `Bank Staff + Cash Staff — ${nm} (grand total)` :
              l==='BANK STAFF TOTAL PAYOUT:' ? `Bank + Cash payout for bank staff — ${nm}` :
              `SUM of ${l.replace(':','')} staff — ${nm}`;
  ...
  navRows.push({kind:'tot',l:'GRAND TOTAL:',t:gT});
      ...
      {pan&&<>
        ...
        <div className="pn np">
          ...
          <div className="pnb">
            ...
            {hiddenStaff.length>0&&<><div style={{fontSize:12,fontWeight:700,margin:'16px 0 8px',textTransform:'uppercase',letterSpacing:'.05em',color:'#a3a3a3'}}>Inactive staff ({hiddenStaff.length})</div>{hiddenStaff.map(s=><div className="si" key={s.id} style={{opacity:0.6}}><div className="sif"><div className="sin">{s.name}</div><div className="sim">{s.position} &middot; RM{s.salary}</div></div><button className="b bg" style={{padding:'4px 8px',fontSize:12,color:'#16a34a'}} onClick={()=>showForMonth(s.id)}>Restore</button><button className="b br" style={{padding:'4px 8px',fontSize:12}} onClick={()=>permDelS(s.id)}>Delete</button></div>)}</>}
